...
## Laravel Configuration + Database
...
I used Laravel sail to deploy my app in a Docker container.
...
Now, we will add a column in the user table to store Ethereum addresses.

For this, you can create a migration.

```shell
php artisan make:migration alter_user_table --table=users
```

How look like this file :
...
Uncomment the file /database/seeders/DatabaseSeeder.php

And let’s launch migrate to deploy the tables and seed to fill the tables.
...
We need ethers library to communicate with Metamask, let’s use the last version (v6).
```shell
npm install ethers
npm run dev
```

Create a new component for the MetaMask login button (/resources/js/Components/LoginMetamaskButton.jsx)
...
You can add this button where you want with this line :
...
It’s ok for the front, let’s create the Laravel route in /routes/auth.php file and the middleware ‘guest’
...
```php:/app/Http/Controllers/Auth/MetamaskAuthController.php
...
Now your users can use their wallet to authenticate, but there is a huge security breach. An attacker can simply use a post request with the Ethereum address to access of the user's backend.

![Schema de l'authentification](/static/images/metamask-auth-laravel-breeze-react-ethers/schema.webp)

We require two library to decode the signature with PHP.
...
## Allow the user to add/modify Ethereum address
A last feature to code is to let the user be able to add or modify his Ethereum address.

```php:/app/Http/Requests/ProfileUpdateRequest.php
'eth_address' => ['string', 'max:255'],
```

```jsx:/resources/js/Pages/Profile/Partials/UpdateProfileInformationForm.jsx
<div>
...
You can find all the code here : https://github.com/geof-dev/laravel-metamask-auth
